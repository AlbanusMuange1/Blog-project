...
function old_select($key, $value, $default = ''){
    if(!empty($_POST[$key]) && $_POST == $value)
        return " selected ";
    if($default == $value)
    return " selected ";
...
// escape to avoid messing the HTML
function esc($str){
...
function get_image($file){
    ...
    return ROOT.'/assets/images/social logo.png'.$file;
}
	...
	if(file_exists($filename))
	{
		...
		switch ($type) {
			...
			case 'image/webp':
				...
				break;
            case 'image/jpg':
                imagejpg($dst_image, $filename, 100);
                break;
